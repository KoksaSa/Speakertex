 ...
 * следует пробел или конец строки. Поэтому десятичные числа вроде «3.5»
 * и сокращения без пробела после точки не разрываются.
 *
 * Примеры:
...
export function splitIntoSentences(text: string): string[] {
  ...
  for (const line of lines) {
    ...
    // lookahead (?=\s|$) гарантирует, что точка внутри числа "3.5" не станет границей
